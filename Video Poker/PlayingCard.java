...
class Decks {
     ...
     */
    public Decks(boolean withJokers) throws PlayingCardException
    {
      try {
      ...
      catch (Exception e){System.out.println("This doesn't work!");}
    ...
    try {
      ...
      if (withJokers){
        ...
        gameDecks = new ArrayList<Card>();  // create new game deck
        gameDecks = originalDecks;          // copy original deck to game deck before shuffling
      
        // implement this method!
    }
      catch (Exception e){System.out.println("This isn't working!");}
    ...
    {
      List<Card> hand = new ArrayList(numberCards);
      if(remainSize() >= numberCards)
         {
           for(int i =0; i < numberCards;i++){
               hand.add(gameDecks.get(i));
               gameDecks.remove(i); 
               }
         }
      else {      
         try {throw new PlayingCardException("There are not enough cards in the deck to deal another hand.");}
                  
         catch (PlayingCardException e){System.out.println("Error 2017: " + e.getMessage());}
        // implement this method!
            }      
         return hand;
     }
    ...
    public String toString()
    {
	return "The current deck is:"+gameDecks;
    }
